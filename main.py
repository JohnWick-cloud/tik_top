...
@dp.message_handler(commands='audio1')
...
async def get_audio(message: types.Message, state: FSMContext):
    if message.media_group_id:
        ...
        btn = InlineKeyboardButton('Слушать', url=f"https://t.me/tiktopaudiobot?start={ message.media_group_id}")
    else:
        response = requests.post(
        url=f'https://api.telegram.org/bot{cfg.TOKEN}/forwardMessage',
        data={f'chat_id': {cfg.CHANELID}, 'from_chat_id': {message.from_user.id}, 'message_id': {message.message_id}}
    ).json()
        btn = InlineKeyboardButton('Слушать', url=f"https://t.me/tiktopaudiobot?start={response['result']['message_id']}")
...
async def done(message: types.Message, state: FSMContext):
    ...
    await bot.send_photo(chat_id= -1001649909625, photo=data['photo_id'], caption=data['caption'], reply_markup=data['keyboard'])
    await message.answer('Готово',reply_markup='')
    await state.finish()



@dp.message_handler(commands='audio2')
async def audio1(message: types.Message):
    await message.answer('Отправьте фото')
...
async def get_doc(message: types.Message, state: FSMContext):
    ...
    btn = InlineKeyboardButton('Скачать книгу', url=f"https://t.me/tiktopaudiobot?start={response['result']['message_id']}")
...
async def done(message: types.Message, state: FSMContext):
    ...
    await bot.send_photo(chat_id= -1001649909625, photo=data['photo_id'], caption=data['caption'], reply_markup=keyboard)
    await message.answer('Готово',reply_markup='')
    await state.finish()
